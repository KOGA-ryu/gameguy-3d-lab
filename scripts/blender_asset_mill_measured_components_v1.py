#!/usr/bin/env python3
"""Render Asset Mill Measured Components v1 review scene.

Run with Blender:

/Applications/Blender.app/Contents/MacOS/Blender --background --python scripts/blender_asset_mill_measured_components_v1.py
...
import compile_asset_mill_measured_components_v1 as compiler  # noqa: E402


INDEX_PATH = ROOT / "goal" / "architecture" / "asset_mill_measured_v1" / "asset_mill_measured_index_v1.json"
OUT_DIR = ROOT / "goal" / "architecture" / "blender_tests"
BLEND_PATH = OUT_DIR / "asset_mill_measured_components_v1.blend"
RENDER_PATH = OUT_DIR / "asset_mill_measured_components_v1_workbench.png"
REPORT_PATH = OUT_DIR / "asset_mill_measured_components_v1_report.json"
...
def make_materials() -> dict[str, bpy.types.Material]:
    return {
        "stone": make_material("measured_warm_stone", (0.56, 0.52, 0.44, 1.0)),
        "support": make_material("measured_support_limestone", (0.75, 0.66, 0.42, 1.0)),
        "walkable": make_material("measured_walkable_slab", (0.30, 0.56, 0.36, 1.0)),
        "rib": make_material("measured_arch_rib", (0.82, 0.72, 0.48, 1.0)),
        "cap": make_material("measured_cap_block", (0.62, 0.58, 0.50, 1.0)),
        "rail": make_material("measured_rail_barrier", (0.30, 0.46, 0.62, 1.0)),
        "socket": make_material("measured_socket_marker", (0.12, 0.40, 0.86, 1.0)),
    }
...
def round_arch_points(span: float, spring_z: float, _rise: float, y: float, segments: int = 28) -> list[tuple[float, float, float]]:
    radius = span * 0.5
    return [
        (
...
def add_curve(part: dict[str, Any], materials: dict[str, bpy.types.Material], parent: bpy.types.Object) -> bpy.types.Object:
    if part["curve_kind"] == "round":
        points = round_arch_points(float(part["span_m"]), float(part["spring_z_m"]), float(part["rise_m"]), float(part["y_m"]))
    else:
...
def add_scene_context() -> None:
    ...
    light.name = "asset_mill_measured_area_light"
...
def main() -> None:
    ...
    report = {
        "schema": "asset_mill_measured_components_blender_report_v1",
