...
- Blender tool-plan compiler validates a `97`-tool dictionary, a `9`-family sequence policy, and compiles nine default plans: a `32`-step banister post, a `32`-step fence post, a `28`-step rail segment, a `57`-step gothic panel guard, a `31`-step column, a `25`-step window frame, a `25`-step door frame, a `22`-step profiled plinth base detail, and a `23`-step profiled plinth/post context prototype.
