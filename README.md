...
The next post-focused reference is `docs/research/component_style_system_v0/railing_post_style_atlas_v0.md`. It redirects near-term railing work to one reusable post with many style variants, keeping rails, infill panels, stair pitch, full railing runs, and building-code claims out of scope until the post language is strong.

The first source-side post matrix is `data/architecture/component_style_sheets/railings/single_post_style_matrix_v0.json`. It declares five comparable variants for one reusable post: a plain square reference, Gothic buttress newel, Gothic clustered shaft, castle crenel cap, and rustic chamfered timber post. Validate it with:
...
Asset readiness planning now lives in `docs/research/asset_readiness_system_v0/`. It defines the practical prep layer before a hands-on Blender pass: exact component selection, reference packets, drawing guides, component terminology, Blender tool workcards, modular game-asset requirements, QA checklists, and correction capture. This is the "operator handoff" lane: Codex prepares the terms, guides, tools, and checklists; the human operator makes the final Blender calls; stable corrections then move back into source recipes or style sheets.

Animation planning now lives in `docs/research/animation_system_v0/`. It defines animation TODOs, rig complexity levels, motion roles, loop types, timing fields, source recipe feedback rules, per-asset animation workcards, Blender animation/rigging tool groups to research later, and a backlog across architecture, mechanisms, furniture, instruments, food/drink, animals, accessories, clothing, armor, weapons, materials, and VFX. It is documentation only; it does not create rigs, actions, keyframes, Blender files, or game-engine controllers.

Chess-engine generation logic research now lives in `docs/research/chess_engine_generation_logic_v0/`. It maps chess-engine ideas such as board state, legal move generation, search, evaluation, pruning, transposition tables, quiescence, perft, and UCI separation onto future source-owned 3D generation. The intended direction is a deterministic generation engine that searches legal operation sequences, scores candidate asset JSON, and lets Blender remain an adapter for preview/export. It is documentation only; it does not implement a generator, run Blender, or change the asset pump.

Cross-lane documentation planning now lives in `docs/research/documentation_map_v0/`. It records the documentation layers still needed across architecture, dungeon/map pieces, furniture, accessories, food/drink, animals, instruments, tools/crafts, textures/materials, decorative geometry, readable objects, and future lanes such as clothing, armor, weapons, plants, vehicles, and signage. The first global style atlas and shape-language dictionary live there too. It is a documentation map only, not a build queue.

Imported non-architecture asset taxonomies now live in `data/asset_taxonomy/` with human triage notes in `docs/research/imported_asset_taxonomies_v0/`. The current imported seeds came from the user's local Documents folder and cover armor history, armor-making materials/tools/techniques, human body proxy chunks, textile/clothing construction, and sewing/textile equipment. They are raw source seeds, not active architecture pipeline inputs; promote terms only after a normalized crosswalk maps them to geometry dictionary terms and Blender tool cards.

The first imported taxonomy crosswalk is `data/asset_taxonomy/normalized_domains_v0/shape_type_crosswalk_v0.json`. It maps representative imported shape/proxy terms such as dome shells, lamellar plate arrays, woven sheet grids, seam curves, stitch dash arrays, capsule proxies, joint proxies, spline plates, measuring ribbons, spools, buckles, and wire spirals to repo geometry terms, Blender tool IDs, source fields, drafting tags, and promotion status. Validate it with `python3 scripts/validate_imported_taxonomy_crosswalk_v0.py`.

Musical-instrument prop planning now lives in `docs/research/musical_instrument_assets_v0/`, with source data at `data/asset_taxonomy/musical_instruments_v0/musical_instrument_asset_taxonomy_v0.json`. This lane documents starter instruments such as lute, Gothic harp, recorder, shawm, frame drum, cast bell, portative organ, and hurdy-gurdy, mapping visible anatomy to geometry terms, Blender tool IDs, drawing UI tags, source fields, operator checks, and in-game lore book hooks. Validate it with `python3 scripts/validate_musical_instrument_taxonomy_v0.py`.

Music-theory and playing-method research now lives in `docs/research/musical_instrument_assets_v0/` too. It maps pitch, mode, rhythm, melody, harmony, texture, notation, tablature, and instrument playing actions into future prop details, readable music pages, animation cues, sound hooks, and operator checks.

Furniture prop planning now lives in `docs/research/furniture_assets_v0/`, with source data at `data/asset_taxonomy/furniture_v0/furniture_asset_taxonomy_v0.json`. This lane documents furniture families, status/caste tiers, styles, and starter objects such as rough plank stools, joined benches, boarded chests, trestle tables, panelled cupboards, high-back chairs, canopy beds, lecterns, workbenches, and thrones. It maps visible anatomy to geometry terms, Blender tool IDs, drawing UI tags, source fields, operator checks, and lore book hooks. Validate it with `python3 scripts/validate_furniture_asset_taxonomy_v0.py`.

Accessory prop planning now lives in `docs/research/accessory_assets_v0/`, with source data at `data/asset_taxonomy/accessories_v0/accessory_asset_taxonomy_v0.json`. This lane documents wearable and carried accessories such as belts, pouches, brooches, rings, pendants, keys, chatelaines, satchels, scabbard hangers, spectacles, seal matrices, and waterskins. It maps visible anatomy and attachment mechanics to geometry terms, Blender tool IDs, drawing UI tags, source fields, operator checks, and lore book hooks. Validate it with `python3 scripts/validate_accessory_asset_taxonomy_v0.py`.

Food/drink prop planning now lives in `docs/research/food_drink_assets_v0/`, with source data at `data/asset_taxonomy/food_drink_v0/food_drink_asset_taxonomy_v0.json`. This lane documents bread, grain, produce, cheese, meat, fish, stews, drink vessels, table service, and pantry storage, with starter props such as loaves, flatbreads, cheese wedges, apples, root bundles, roasted meat, fish platters, stew bowls, tankards, goblets, jugs, bottles, grain sacks, spice jars, trenchers, flasks, and spoiled fruit scatter. It maps visible anatomy and serving/storage state to geometry terms, Blender tool IDs, drawing UI tags, source fields, operator checks, and lore book hooks. Validate it with `python3 scripts/validate_food_drink_asset_taxonomy_v0.py`.

Cooking and recipe method research now lives in `docs/research/food_drink_assets_v0/` too. It maps recipe manuscripts, household receipt books, feast menus, prep stations, hearth/cauldron cooking, roasting, baking, frying, drying, curing, smoking, pickling, fermentation, preserves, dairy, brewing, and pantry/cellar states into visible asset cues, source fields, Blender direction, operator checks, and readable lore hooks.

Animal asset planning now lives in `docs/research/animal_assets_v0/`, with source data at `data/asset_taxonomy/animals_v0/animal_asset_taxonomy_v0.json`. This lane documents animal role tiers, body-plan styles, families, and starter animals such as village dogs, barn cats, riding horses, pack donkeys/mules, cattle/oxen, sheep/goats, pigs/boars, chickens, rats, bats, deer, wolves, crows/ravens, fish, snakes, and frogs/toads. It maps visible anatomy and locomotion read to geometry terms, Blender tool IDs, drawing UI tags, source fields, operator checks, and lore book hooks. Validate it with `python3 scripts/validate_animal_asset_taxonomy_v0.py`.

Craft fabrication method notes now live in `docs/research/craft_fabrication_methods_v0/`, with source data at `data/architecture/taxonomy/craft_methods/craft_fabrication_methods_v0.json`. This lane maps real craft methods such as tracing-floor templates, stone dressing, tracery layout, arch centering/voussoirs, rib vault ribs/webbing/bosses, timber mortise-and-tenon frames, moulding profile planes, wrought-iron scroll bending, rivet/collar joints, and metal relief work into drawing UI tags, source fields, Blender tool IDs, and operator checks. Validate it with `python3 scripts/validate_craft_fabrication_methods_v0.py`.

Broader cross-family construction method research also lives in `docs/research/craft_fabrication_methods_v0/`. It maps building masonry/roof/plaster/timber methods, furniture joinery/carving/bentwood/veneer/upholstery, musical instrument bodies/strings/membranes/bells, metalwork, ceramics, textiles, leather-like accessories, and woven natural forms into visible asset cues, source fields, Blender direction, and operator checks.
